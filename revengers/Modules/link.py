...
from pyrogram.types import Message
from revengers import bot
from revengers.db import get_chakra, remove_chakra
import re
...
async def link_handler(bot, message: Message):
    ...
    if re.search(INSTA_REGEX, text) or re.search(YT_REGEX, text):
        ...
        chakra = await get_chakra(user_id)
        if chakra >= 200:
            ...
            try:
                ...
                video_file = await download_video_from_link(text)  # You must define this function
